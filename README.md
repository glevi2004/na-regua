...
**43 dos 102 dias-desenvolvedor que faltam estão bloqueados por 11 decisões em
...
`NR-022`) e a movimentação de estoque (`NR-023`). O próximo passo esbarra numa
tarefa que **falta no ledger**: nenhuma cria os repositórios Postgres por trás
das portas de `core`, e sem eles as rotas de `api` só se ligariam a _fakes_.
